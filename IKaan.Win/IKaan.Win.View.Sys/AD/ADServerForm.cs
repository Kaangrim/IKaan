...
{
	public partial class ADServerForm : EditForm
	{
		public ADServerForm()
		{
			InitializeComponent();
		...
		void InitGrid()
		{
			...
			gridList.AddGridColumns(
				...
				new XGridColumn() { FieldName = "ServerName", Width = 300 },
		...
		{
			gridList.BindList<ADServer>("AD", "GetList", "Select", new DataMap() { { "FindText", txtFindText.EditValue } });
		...
		void DetailDataLoad(object id)
		{
			try
			{
				var model = WasHandler.GetData<ADServer>("AD", "GetData", "Select", new DataMap() { { "ID", id } });
				if (model == null)
		...
		protected override void DataSave(object arg, SaveCallback callback)
		{
			try
			{
				var model = this.GetControlData<ADServer>();
				using (var res = WasHandler.Execute<ADServer>("AD", "Save", (this.EditMode == EditModeEnum.New) ? "Insert" : "Update", model, "ID"))
				{
		...
		protected override void DataDelete(object arg, DeleteCallback callback)
		{
			try
			{
				...
				using (var res = WasHandler.Execute<DataMap>("AA", "Delete", "DeleteADServer", map, "ID"))
				{
					if (res.Error.Number != 0)
						throw new Exception(res.Error.Message);
