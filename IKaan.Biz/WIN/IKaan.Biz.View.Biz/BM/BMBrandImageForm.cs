...
{
	public partial class BMBrandImageForm : EditForm
	{
		private string loadUrl = string.Empty;

		public BMBrandImageForm()
		{
			InitializeComponent();
		...
		private void UploadImage()
		{
			try
			{
				...
				BMBrandImage model = new BMBrandImage()
				{
				...
				using (var res = WasHandler.Execute<BMBrandImage>("BM", "Save", (this.EditMode== EditModeEnum.New)?"Insert":"Update", model, "ID"))
				{
		...
		private void DeleteImage()
		{
			try
			{
				...
				using (var res = WasHandler.Execute<DataMap>("BM", "Delete", "DeleteBMBrandImage", map, "ID"))
				{
					if (res.Error.Number != 0)
						throw new Exception(res.Error.Message);
