...
namespace IKaan.Win.View.Biz.BG
{
	public partial class BGGoodsEditForm : EditForm
	{
		public BGGoodsEditForm()
		{
			...
			btnCreateOption.Click += (object sender, EventArgs e) =>
			{
				try
				{
					...
					if (gridItem.DataSource == null)
						gridItem.DataSource = new List<BGGoodsItem>();
					...
					if (option1.IsNullOrEmpty() == false && option2.IsNullOrEmpty() == false)
					{
						foreach (string opt1 in option1.Split(','))
						{
							...
							{
								if ((gridItem.DataSource as List<BGGoodsItem>).Where(x => x.Option1Name == opt1 && x.Option2Name == opt2).Any() == false)
								{
									gridItem.AddRow(new BGGoodsItem()
									{
					...
					else
					{
						if (option1.IsNullOrEmpty() == false)
						{
							foreach (string opt1 in option1.Split(','))
							{
								if ((gridItem.DataSource as List<BGGoodsItem>).Where(x => x.Option1Name == opt1).Any() == false)
								{
									gridItem.AddRow(new BGGoodsItem()
									{
										Option1Type = lupOption1Type.EditValue.ToString(),
						...
						else if (option2.IsNullOrEmpty() == false)
						{
							foreach (string opt2 in option2.Split(','))
							{
								if ((gridItem.DataSource as List<BGGoodsItem>).Where(x => x.Option2Name == opt2).Any() == false)
								{
									gridItem.AddRow(new BGGoodsItem()
									{
										Option2Type = lupOption2Type.EditValue.ToString(),
			...
			btnSaveImage.Click += (object sender, EventArgs e) =>
			{
				...
				if (gridImage.DataSource == null)
					gridImage.DataSource = new List<BGGoodsImage>();

				gridImage.MainView.BeginUpdate();
				(gridImage.DataSource as List<BGGoodsImage>).Add(new BGGoodsImage()
				{
		...
		protected override void DataInit()
		{
			...
			gridItem.Clear<BGGoodsItem>();
			gridImage.Clear<BGGoodsImage>();
			gridAttribute.Clear<BGGoodsAttribute>();
			gridInfoNotice.Clear<BGGoodsInfoNotice>();
		...
		protected override void DataLoad(object param = null)
		{
			...
			try
			{
				...
				if (model.Detail == null)
					model.Detail = new BGGoodsDetail();
				if (model.Item == null)
					model.Item = new List<BGGoodsItem>();
				if (model.Image == null)
					model.Image = new List<BGGoodsImage>();
				if (model.Attribute == null)
					model.Attribute = new List<BGGoodsAttribute>();
				if (model.InfoNotice == null)
					model.InfoNotice = new List<BGGoodsInfoNotice>();
		...
		protected override void DataSave(object arg, SaveCallback callback)
		{
			...
			try
			{
				...
				model.Item = (gridItem.DataSource == null) ? null : gridItem.DataSource as List<BGGoodsItem>;				
				model.Attribute = (gridAttribute.DataSource == null) ? null : gridAttribute.DataSource as List<BGGoodsAttribute>;
				model.InfoNotice = (gridInfoNotice.DataSource == null) ? null : gridInfoNotice.DataSource as List<BGGoodsInfoNotice>;
				model.Image = null;
				...
				model.Image = (gridImage.DataSource == null) ? null : gridImage.DataSource as List<BGGoodsImage>;

				//이미지 파일저장
				if (model.Image != null && model.Image.Count > 0) {
					//FTP저장
					foreach (BGGoodsImage image in model.Image.Where(x => x.RowState == "INSERT"))
					{
					...
					//이미지정보 저장
					using (var res = WasHandler.Execute<BGGoodsImage>("BG", "Save", (this.EditMode == EditModeEnum.New) ? "Insert" : "Update", model, "ID"))
					{
						if (res.Error.Number != 0)
							throw new Exception(res.Error.Message);
					}
		...
		private void DeleteOption()
		{
			try
			{
				...
				else
				{
					...
					using (var res = WasHandler.Execute<DataMap>("BG", "Delete", "DeleteBGGoodsItem", map, "ID"))
					{
						if (res.Error.Number != 0)
							throw new Exception(res.Error.Message);
		...
		private void DeleteImage()
		{
			try
			{
				...
				else
				{
					...
					using (var res = WasHandler.Execute<DataMap>("BG", "Delete", "DeleteBGGoodsImage", map, "ID"))
					{
						if (res.Error.Number != 0)
							throw new Exception(res.Error.Message);
		...
		private void LoadInfoNotice()
		{
				...
				DataMap map = new DataMap()
			{
				...
				gridInfoNotice.BindList<BGGoodsInfoNotice>("BG", "GetList", "SelectBGGoodsInfoNoticeByCategory", map);
			}
			catch (Exception ex)
			{
				ShowErrBox(ex);
