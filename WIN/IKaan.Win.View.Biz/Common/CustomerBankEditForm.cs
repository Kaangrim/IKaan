	...
	public partial class CustomerBankEditForm : EditForm
	{
		...
		protected override void DataLoad(object param = null)
		{
			...
			try
			{
				...
				var model = WasHandler.GetData<CustomerBankModel>("Biz", "GetData", "Select", parameter);
				if (model == null)
		...
		{
			ClearControlData<CustomerBankModel>();
			txtCustomerID.EditValue = (this.ParamsData as DataMap).GetValue("CustomerID");
			txtCustomerID.EditText = (this.ParamsData as DataMap).GetValue("CustomerName");
		...
		protected override void DataSave(object arg, SaveCallback callback)
		{
			try
			{
				var model = this.GetControlData<CustomerBankModel>();
				...
				using (var res = WasHandler.Execute<CustomerBankModel>("Biz", "Save", (this.EditMode == EditModeEnum.New) ? "Insert" : "Update", model, "ID"))
				{
		...
		protected override void DataDelete(object arg, DeleteCallback callback)
		{
			try
			{
				...
				using (var res = WasHandler.Execute<DataMap>("Biz", "Delete", "DeleteCustomerBank", map, "ID"))
				{
					if (res.Error.Number != 0)
						throw new Exception(res.Error.Message);
