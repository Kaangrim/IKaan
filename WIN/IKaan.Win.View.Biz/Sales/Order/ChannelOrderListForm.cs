	...
	public partial class ChannelOrderListForm : EditForm
	{
		...
		void InitGrid()
		{
			try
			{
				...
				gridOrders.AddGridColumns(
					...
					new XGridColumn() { FieldName = "CouponAmt", Width = 70, FormatType = FormatType.Numeric, FormatString = "N0", HorzAlignment = HorzAlignment.Far },
		...
		protected override void DataLoad(object param = null)
		{
			try
			{
				if (lcTab.SelectedTabPage.Name == lcGroupOrder.Name)
				{
					...
					gridOrders.BindList<ChannelOrderModel>("Biz", "GetList", "Select", parameter);
				}
				else if (lcTab.SelectedTabPage.Name == lcGroupCancel.Name)
				{
					...
					gridOrders.BindList<ChannelOrderCancelModel>("Biz", "GetList", "Select", parameter);
				}
				else if (lcTab.SelectedTabPage.Name == lcGroupReturn.Name)
				{
					...
					gridOrders.BindList<ChannelOrderReturnModel>("Biz", "GetList", "Select", parameter);
				}
			}
			catch(Exception ex)
			{
		...
		private void UploadChannelOrder()
		{
			try
			{
				...
				var data = UploadHandler.GetExcelData<ChannelOrderModel>(fileName, startLine);
				...
				{
					line.StoreID = lupStoreID.EditValue.ToIntegerNullToZero();
		...
		void SaveOrder()
		{
			try
			{
				...
				using (var res = WasHandler.Execute<ChannelOrderModel>("Biz", "Save", "UpdateChannelOrderBrand", list, "ID"))
				{
		...
		void ApplyOrder()
		{
			try
			{
				...
				using (var res = WasHandler.Execute<ChannelOrderModel>("Biz", "Save", "InsertChannelOrderToBizOrder", list, "ID"))
				{
					if (res.Error.Number != 0)
						throw new Exception(res.Error.Message);
				}
