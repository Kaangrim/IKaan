...
namespace IKaan.Was.Service.BIZ
{
	public static class BTService
	{
		public static WasRequest GetList(WasRequest request)
		{
			try
			{
				...
				foreach (WasRequest req in list)
				{
					...
					{
						case "BTOrderSum":
							req.SetList<BTOrderSum>();
							break;
						case "BTOrderSumSales":
							req.SetList<BTOrderSumSales>();
							break;
					}
				}
		...
		private static void SaveOrderSumByChannel(this WasRequest req)
		{
			try
			{
				int count = 0;
				BTOrderSumByChannel model = req.Data.JsonToAnyType<BTOrderSumByChannel>();
				if (model != null)
				{
